...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="grid min-h-screen grid-cols-1 bg-black/15">
        <section className="flex min-w-0 flex-col p-6 md:p-10">
          <header className="mb-20 flex items-center justify-between">
            ...
            {isLoggedIn ? (
              ...
              <Link
                href="/login"
                className="rounded-xl border border-white/15 px-4 py-2 text-sm text-muted transition hover:bg-white/5 hover:text-foreground"
              >
                Login
              </Link>
            )}
          ...
          <div className="max-w-xl space-y-6">
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <Link
                href={workspaceHref}
                className="rounded-xl border border-white/15 bg-white/10 px-5 py-3 text-sm font-medium transition hover:bg-white/15"
              >
                Open Workspace
              </Link>
            </div>
